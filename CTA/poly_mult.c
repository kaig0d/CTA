...
void mult_minus_rqhat(POLY_QHAT *c, const POLY_QHAT *a, const POLY_QHAT *b)
{
	...
	for (i = 0; i < D; i++)
	{
		c->poly[i] = c->poly[i] - tmp.poly[i] + QHAT;
	}	
}

/* c=c+a*b when pm=0 or c=c-a*b when pm=1 on R_{\hat{q}} */
void mult_plus_rqhat_pm(POLY_QHAT *c, const POLY_QHAT *a, const POLY_QHAT *b, const uint64_t pm)
{
	POLY_QHAT tmp;
	uint64_t i;
	...
	for (i = 0; i < D; i++)
	{
		c->poly[i] += (1 - 2 * pm) * tmp.poly[i] + QHAT;
	}	
}
